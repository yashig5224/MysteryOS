  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        console.error("Copilot dataset fetch error:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#f8fafc] text-slate-900">
      <Header
        ...
        subtitle="Grounded RAG reasoning engine with multi-hop source citations and deterministic dossiers"
        badge={<Badge variant="primary">RAG ONLINE</Badge>}
      />

      <PageContainer className="py-8 space-y-6">
        <div className="space-y-1">
          <h2 className="text-base font-bold text-slate-900">Select an Active Investigation Target</h2>
          <p className="text-xs text-slate-600">
            Open any dataset to start querying empirical evidence, hypothesis formulations, and anomaly distributions.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <Card className="p-12 text-center bg-white border-slate-200 shadow-sm space-y-3">
            <h3 className="text-sm font-bold text-slate-900">No Ingested Datasets</h3>
            <p className="text-xs text-slate-600 max-w-sm mx-auto">
              Upload a dataset to activate the AI copilot and ask analytical questions.
            </p>
